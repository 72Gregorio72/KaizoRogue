...
// .text
u16 GetStarterPokemon(u16 starterNum)
{
    if (starterNum > 2)
        starterNum = 0;

    if (gSaveBlock2Ptr->randomStarters[starterNum] != SPECIES_NONE)
        return gSaveBlock2Ptr->randomStarters[starterNum];

    // Fallback vanilla se non inizializzato
    switch (starterNum)
    {
    case 0:
