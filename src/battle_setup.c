...
static u16 GetKaizoRandomTrainerSpecies(u16 originalSpecies, u16 trainerNum, u8 monIndex)
{
    ...
    do {
        ...
        newSpecies = 1 + ((rng >> 16) % (NUM_SPECIES - 1));
...
static void CreateNPCTrainerParty(struct Pokemon *party, u16 trainerNum)
{
    ...
    // --- CONTROLLO LOTTA LABORATORIO OAK ---
    // Se la lotta è quella del laboratorio, il rivale DEVE usare la sfera presa dal tavolo
    if (trainerNum == TRAINER_RIVAL_OAKS_LAB_SQUIRTLE 
     || trainerNum == TRAINER_RIVAL_OAKS_LAB_BULBASAUR 
     || trainerNum == TRAINER_RIVAL_OAKS_LAB_CHARMANDER)
    {
        u16 rivalStarter = gSaveBlock2Ptr->rivalStarterSpecies;
        if (rivalStarter != SPECIES_NONE)
        {
            SetMonData(&party[0], MON_DATA_SPECIES, &rivalStarter);
            SetMonData(&party[0], MON_DATA_NICKNAME, GetSpeciesName(rivalStarter));
            GiveMonInitialMoveset(&party[0]);
            CalculateMonStats(&party[0]);
            return; // Esci subito, evita di ri-randomizzarlo sotto!
        }
    }

    // --- RANDOMIZZAZIONE GENERALE ALTRI TRAINER ---
    for (u8 i = 0; i < PARTY_SIZE; i++)
    {
        u16 currentSpecies = GetMonData(&party[i], MON_DATA_SPECIES);
        if (currentSpecies == SPECIES_NONE || currentSpecies == SPECIES_EGG)
            continue;

        u16 newSpecies = GetKaizoRandomTrainerSpecies(currentSpecies, trainerNum, i);
