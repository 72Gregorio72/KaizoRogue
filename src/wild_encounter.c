...
static enum Species GetKaizoRandomWildSpecies(enum Species originalSpecies, u8 level)
{
    ...
    // Combina il seed della run con la mappa, la specie originale e il livello
    u32 encounterSeed = seed ^ (gSaveBlock1Ptr->location.mapGroup << 16)
                             ^ (gSaveBlock1Ptr->location.mapNum << 8)
                             ^ (originalSpecies * 7919)
                             ^ (level * 37);

    u32 rng = encounterSeed;
    enum Species newSpecies;
    do {
        rng = 1103515245 * rng + 12345;
        newSpecies = (enum Species)(1 + ((rng >> 16) % (NUM_SPECIES - 1)));
...
{
    // --- HOOK KAIZO RANDOM ENCOUNTERS ---
    species = GetKaizoRandomWildSpecies(species, level);
