...
static const u8 sDefaultPlayerName[] = _("RED");
...
static void WarpToTruck(void)
{
    // Il centro della mappa 20x20 è all'incirca coordinate X: 10, Y: 10
    SetWarpDestination(MAP_GROUP(MAP_KAIZO_HUB), MAP_NUM(MAP_KAIZO_HUB), WARP_ID_NONE, 10, 10);
    WarpIntoMap();
}
...
void SeedCustomRng(u32 seed) {
    sRandomizerRng = seed;
}

u16 CustomRandom(void) {
...
{
#if IS_FRLG
    static const u8 sDefaultRivalName[] = _("BLUE");
...
#if IS_FRLG
    StringCopy(rivalName, sDefaultRivalName);
#endif
